...
|---|---|---|
| `~/.hermes/memories/MEMORY.md` | Рабочая память — только актуальное, чистится | Обновлять через `memory` tool |
| `~/.hermes/full-time-memory.md` | **Вечный журнал** — вся хроника, только append | Читать первым делом при старте |
| `~/.hermes/SOUL.md` | DevOps-личность | Не менять |
| `~/.hermes/AGENTS.md` | Этот файл — карта инфраструктуры | Не менять |
